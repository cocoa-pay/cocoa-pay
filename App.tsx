...
const App: React.FC = () => {
  ...
  const loadData = async (key: string, sources: SubsidySource[]) => {
    if (!key) {
    ...
    setApiError(null);
    try {
      const realData = await fetchSubsidies(key, sources);
  ...
  const handleToggleSource = (source: SubsidySource) => {
    ...
    // Reload data if we are in Real API mode to save bandwidth or filter correctly
    if (apiKey) {
        loadData(apiKey, newSources);
    }
  };
  ...
  const handleOnboardingComplete = (profile: UserProfile) => {
    ...
    if (apiKey) {
      loadData(apiKey, selectedSources).then(() => setHasOnboarded(true));
    } else {
      setTimeout(() => {
        setIsLoading(false);
        ...
        {isLoading && (
          <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center animate-fadeIn">
            ...
            <p className="text-gray-500">
              {isUsingRealApi ? '공공데이터포털 실시간 연동 중...' : '정부24, 복지로, 공공데이터포털에서'}<br />
              선택하신 {selectedSources.join(', ')} 데이터를 조회 중입니다...
            </p>
          </div>
        )}
